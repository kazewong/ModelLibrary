...
train_index = (cum_length[train_index[1]][...,None].repeat(n_sample_points,axis=2)+train_index[0]).reshape(-1,n_hyper_points*n_sample_points).astype(int)
...
valid_index = (cum_length[valid_index[1]][...,None].repeat(n_sample_points,axis=2)+valid_index[0]).reshape(-1,n_hyper_points*n_sample_points).astype(int)
...
model = DeepSets_Siren_Weight(in_features=3, out_features=1, hidden_features=128, hidden_layers=3,set_features=128)
...
#    print(epoch)
    ...
    for index in range(indices.shape[0]//batch_size+1):
        locations = train_context[index*batch_size:(index+1)*batch_size]
        values = train_y[train_index][index*batch_size:(index+1)*batch_size]
        query = train_x[index*batch_size:(index+1)*batch_size]
        outputs = train_y[index*batch_size:(index+1)*batch_size]
        optimizer.zero_grad()

        pred_y = model(locations,values,query)
#        pred_y = model(inputs)
        loss = torch.mean(torch.sum(((pred_y-outputs)/(outputs))**2,dim=1))
#        loss = torch.mean(torch.sum(((pred_y-outputs))**2,dim=1))
        loss.backward()
    ...
    if epoch%1==0:
        with torch.no_grad():
            pred_y = model(valid_context,train_y[valid_index],valid_x)
            #pred_y = model(valid_x)
            valid_loss_temp = torch.mean(torch.sum(((pred_y-valid_y)/valid_y)**2,dim=1))
#            valid_loss_temp = torch.mean(torch.sum(((pred_y-valid_y))**2,dim=1))
