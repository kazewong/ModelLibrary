...
class DiffusionDataset(Dataset):
    def __init__(self, path: str, transform=None, has_conditional=False):
        h5_file = h5py.File(path, "r")
        self.has_conditional = has_conditional
        self.transform = transform

        self.data: h5py.Dataset = h5_file["data"]
        ...
        if has_conditional:
            assert isinstance(
                self.conditional_data, h5py.Dataset
            ), "Conditional data is not a dataset"
